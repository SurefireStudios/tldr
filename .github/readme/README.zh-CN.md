...
| | 基线 | 启用 tldr | |
| --- | ---: | ---: | --- |
| 平均输出 token | 339 | **283** | −16% |
| 中位数 | 295 | **186** | −37% |
| 正确性 | 4.771 | **4.979** | +0.208 |
| 保真度 | 4.521 | **4.750** | +0.229 |
| 可执行性 | 4.312 | **4.896** | +0.583 |

token 更少，且每个维度都更好，零阻断性问题，**发布门槛 5 条规则全部通过**。

但请连同前提一起看：基线每次重跑都会重新生成，这次偏低，所以约四分之一的提升来自对照点下移而非技能本身；48 组配对的标准误约 0.090；候选在 34 组胜出、11 组落后——是平均更好，不是每次都好。而且这只是单模型上的一次运行。
