...
| | ベースライン | tldr あり | |
| --- | ---: | ---: | --- |
| 出力トークン（平均） | 370 | **283** | −24% |
| 中央値 | 322 | **175** | −46% |
| エージェント間 | 208 | **101** | −51% |
| 実行可能性 | 4.375 | **4.688** | +0.312 |
| 忠実性 | 4.667 | **4.542** | **−0.125** |

**リリース基準は現在 FAILED** です（5 項目中 2 項目が不合格）。忠実性が 0.025 だけ許容範囲に届いていません。この失敗は調整して消さず、そのまま残しています。「何かを削ったからこそ良く見える回答」を捕まえるために忠実性という項目があり、いまそれがこのスキル自身を捕まえているからです。
