...
16 ケース × 3 試行、スキルなしのベースラインに対するブラインド採点を、同一の計測器で 2 つのモデルに対して実施。公開中のスキル（v0.3.0）の数値です:

| | Sonnet | | Opus | |
| --- | ---: | --- | ---: | --- |
| 正確性 | 4.854 → **4.938** | +0.083 | 4.792 → **4.917** | +0.125 |
| 忠実性 | 4.583 → **4.792** | +0.208 | 4.625 → **4.729** | +0.104 |
| 実行可能性 | 4.396 → **4.750** | +0.354 | 4.396 → **4.875** | +0.479 |
| 安全性 | 4.604 → **4.750** | +0.146 | 4.562 → **4.896** | +0.333 |
| エージェント間の出力トークン | 194 → **122** | −37% | 337 → **139** | −59% |
| 人向け出力トークン（中央値） | 311 → **269** | −13% | 455 → **258** | −43% |
| 人向け出力トークン（平均） | 336 → 372 | +11% | 529 → 558 | +5% |
| 常時オン 1 ターンあたりのスキル自体のトークン | 4,380 → **1,078** | −75% | 4,380 → **1,078** | −75% |

両モデルですべての項目が改善し、**リリース基準は両モデルで 5 項目すべて通過**。ただしトークンの行は注意して読んでください。スキルが圧縮する場面——エージェント間の報告——では出力が 3 分の 1 から半分に減ります。スキルが圧縮を拒む場面——破壊的操作、セキュリティ上の発見、コスト、エラー——では出力はむしろ長くなります。モデルが注意書きを削らず、すべて折りたたみの上に置くようになったからです。平均が上がり中央値が下がるのはそのためで、該当ケースはいずれも忠実性か安全性のスコアが上がっています。いちばん大きい数字はスキル自体のサイズです。常時オンのハーネスは毎ターンこれを送り直すため、旧 18k 版は節約分の約 80 倍のコストを毎ターン払っていました。

ここに至るまで 13 回の実行が必要でした。失敗した回も、原因が私自身のハーネスにあった回も含みます。手法とリリース基準、13 回すべての結果は [`evals/RESULTS.md`](../../evals/RESULTS.md) にあります。数字はスキルに有利かどうかに関わらず公開します。

## 対応エージェント

Claude Code、Cursor、Cybara、OpenClaw、Hermes、Codex、Gemini CLI、GitHub Copilot、OpenCode、Zed、Windsurf、Cline、Roo Code、Aider、Amp、Qwen Code、Kimi Code CLI、Pi、Oh My Pi、Antigravity —— 詳しい手順は [INSTALL.md](../../INSTALL.md) にあります。
