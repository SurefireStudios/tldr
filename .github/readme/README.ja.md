...
16 ケース × 3 試行、スキルなしのベースラインに対するブラインド採点（`claude-sonnet-5`）:

| | ベースライン | tldr あり | |
| --- | ---: | ---: | --- |
| 出力トークン（平均） | 339 | **283** | −16% |
| 中央値 | 295 | **186** | −37% |
| 正確性 | 4.771 | **4.979** | +0.208 |
| 忠実性 | 4.521 | **4.750** | +0.229 |
| 実行可能性 | 4.312 | **4.896** | +0.583 |

トークンが減り、**かつ**すべての項目で改善。ブロッカーはゼロで、リリース基準は 5 項目すべて通過しています。

ただし前提も一緒に読んでください。ベースラインは実行ごとに生成し直すもので、今回は低めに出ました。つまり伸びのおよそ 4 分の 1 は比較対象が下がった分であり、スキル自体の改善ではありません。48 ペアでの標準誤差は約 0.090。候補は 34 ペアで勝ち、11 ペアで負けています。そしてこれは単一モデルでの 1 回の実行です。
